...
contract FixedSwap is Pausable, Whitelist {
    ...
    ERC20 public erc20;
    ERC20 public tokenTrade; /* trade by erc20*/
    bool public isSaleFunded = false;
    uint public decimals = 0;
    bool public isTokenTrade = false;
    ...
    constructor(address _tokenAddress, uint256 _tradeValue, uint256 _tokensForSale, uint256 _startDate,
        ...
        uint256 _feeAmount, address _feeAddress, bool _hasWhitelisting, address _tokenTradeAddress, bool _isTokenTrade
    ) public Whitelist(_hasWhitelisting) {


        require(block.timestamp < _endDate, "End Date should be further than current date");
        require(block.timestamp < _startDate, "start Date should be earlier than current date");
        ...
        if(isTokenTrade){
          tokenTrade = ERC20(_tokenTradeAddress);
        }
    ...
    function cost(uint256 _amount) public view returns (uint){
        return _amount.div(10**decimals).mul(tradeValue);
    }
    ...
    function getPurchase(uint256 _purchase_id) external view returns (uint256, address, uint256, uint256, bool, bool){
        ...
        return (purchase.amount, purchase.purchaser, purchase.costAmount, purchase.timestamp, purchase.wasFinalized, purchase.reverted);
    }
    ...
    /* Fund - Pre Sale Start */
    function fund(uint256 _amount) isSalePreStarted public {
    ...
    function swap(uint256 _amount) payable external whenNotPaused isFunded isSaleOpen onlyWhitelisted {
        ...
        if(isTokenTrade){
          value = cost(_amount);
          require(tokenTrade.transferFrom(msg.sender, address(this), value), "ERC20 Token Transfer Failed");
        }else{
          value = msg.value;
        ...
        if(isTokenSwapAtomic){
            /* Confirm transfer */
            require(erc20.transfer(msg.sender, _amount), "ERC20 Transfer Failed");
        }
        ...
        /* Create new purchase */
        Purchase memory purchase = Purchase(_amount, msg.sender, value, block.timestamp, isTokenSwapAtomic, false);
    ...
    function redeemGivenMinimumGoalNotAchieved(uint256 purchase_id) external isSaleFinalized isNotAtomicSwap {
        ...
        if(isTokenTrade){
          require(tokenTrade.transfer(msg.sender, purchases[purchase_id].costAmount),"Redeem ERC20 Token Transfer Failed");
        }else{
          payable(msg.sender).transfer(purchases[purchase_id].costAmount);
        }
    ...
    function withdrawFunds() external onlyOwner whenNotPaused isSaleFinalized {
        ...
        if(isTokenTrade){
          require(tokenTrade.transfer(FEE_ADDRESS, tokenTrade.balanceOf(address(this)).mul(feePercentage).div(100)), "ERC20 Transfer Failed");
          require(tokenTrade.transfer(msg.sender, tokenTrade.balanceOf(address(this))), "ERC20 Transfer Failed");
        }else{
